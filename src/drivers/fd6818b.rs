...
/// 01=0.4s, 10=0.8s, 11=1.6s per attempt).
...
///
/// The stock routine is `front_end(band, mode)` with three modes, and each
/// mode touches *only one* of two independent 2-way switches:
///
/// | mode | switch driven      | low bands (<300MHz) | high bands (>=300MHz) |
/// |------|--------------------|---------------------|-----------------------|
/// | 0    | both, all released | -                   | -                     |
/// | 1 RX | GPIO5 / GPIO6      | GPIO5=1, GPIO6=0    | GPIO6=1, GPIO5=0      |
/// | 2 TX | GPIO3 / GPIO4      | GPIO3=1, GPIO4=0    | GPIO4=1, GPIO3=0      |
///
/// So GPIO5/GPIO6 is the **RX** band switch and GPIO3/GPIO4 is the **TX**
/// band switch
///
/// The band grouping is `front_end`'s own caller-side classifier
/// (`0x8006568`): `<200MHz` = 0, `200-300` = 2, `300-400` = 3, `>=400` = 1,
/// and the routine groups {0,2} against {1,3} -- i.e. a plain split at
/// 300MHz, which is exactly what `device::radio::Band::from_freq_hz` already
/// does, so `Band` maps onto this table directly.
const REG_GPIO: u8 = 0x33;
const GPIO3: u16 = 0x08; // TX switch, low bands (<300MHz)
const GPIO4: u16 = 0x10; // TX switch, high bands (>=300MHz)
const GPIO5: u16 = 0x20; // RX switch, low bands (<300MHz)
const GPIO6: u16 = 0x40; // RX switch, high bands (>=300MHz)
...
/// | [6]   | reserved                                                  |
/// | [5:3] | `padrv_gain`, 111 = max .. 000 = min (BK4819 numbering)   |
/// | [2:0] | `pa_gain_vreg`, 111 = max .. 000 = min (BK4819 numbering) |
///
/// The low byte here is BK4829's own fixed gain word: not varied by power
/// level, only by band. The real per-radio/per-band level control is the APC
/// target in the top byte, read from the SPI-flash TX power tables
/// (0xF000-0xF0A0).
///
/// **This gain word tracks the vendor codebase, not the RF chip** -- worth
/// stating because two earlier revisions of this file each got it wrong in
/// a different direction. The two available lineages disagree, and each is
/// internally consistent across the axis you would expect to matter:
///
/// | lineage                            | gain word                     | indexed by      |
/// |------------------------------------|-------------------------------|-----------------|
/// | Baofeng SDK (K6 + 5RH, BK4819)     | `{0xFF, 0xD7, 0xFF}`, off `0x7F` | `txPower % 3` |
/// | Quansheng SDK (K5/K1, **both** chips) | `0x88` <280MHz / `0xA2` above | frequency      |
///
/// Two controlled comparisons pin this down:
///
/// - **Chip changes, value doesn't**: `uv-k1-k5v3-firmware-custom` carries
///   `bk4819.c` and `bk4829.c` side by side for the same board. That port
///   is real, not a copy -- it retunes `REG_37` (`0x9D1F`/`0x9F1F` against
///   `0x1D0F`/`0x1F0F`) among ~600 changed lines -- yet it leaves REG 0x36's
///   gain word byte-identical, commenting the original expression out and
///   substituting the same values in hex.
/// - **Chip same, value differs**: Baofeng's BK4819 boards use
///   `{0xFF, 0xD7, 0xFF}` (K6's stock `Rfic_SetPA()` in
///   `src/Driver/DevFD6818.c`, and the 5RH stock firmware disassembled at
///   `nrf014.dis` @ `0x8009ddc`, table `0x0800e0e0` -- the same vendor
///   function byte for byte, the 5RH only adding a `0xC3` carve-out at the
///   Low index inside 155-175MHz), while Quansheng's BK4819 board uses
///   `0x88`/`0xA2`.
///
/// So neither "copy the stock firmware for this board" nor "copy the driver
/// for this chip" is automatically right. The values below are the
/// Quansheng ones, kept identical to K6's own `fd6818b.rs` so the two Rust
/// firmwares share one recipe; on this board they measure at the rated
/// 8W/5W/2W across the three power levels, which is the actual reason to
/// trust them.
///
/// Note also that Baofeng's per-power indexing is a refinement *on top of*
/// the APC target, not the power control itself: its table is
/// `{High: 0xFF, Low: 0xD7, Mid: 0xFF}` -- only the Low entry deviates, i.e.
/// "max drive, except back the internal driver off at low power". The level
/// itself comes from `<15:8>` either way.
const REG_PA: u8 = 0x36;
/// PACTL disabled, gain forced to the UHF word (don't-care while gated
/// off).
const PA_OFF_VALUE: u16 = 0x0022;
/// PACTL on (bit7) + BK4829's UHF driver gain (bit[5:0] = 0x22).
const PA_GAIN_UHF: u16 = 0x00A2;
/// PACTL on (bit7) + BK4829's VHF driver gain (bit[5:0] = 0x08).
const PA_GAIN_VHF: u16 = 0x0088;

#[derive(Clone, Copy, PartialEq, Eq)]
#[allow(dead_code)] // power-level selection not yet exercised by the plain-FM smoke test
pub enum Power {
    Low,
    Mid,
    High,
...
/// own `Rfic_SetAfout()` (`src/Driver/DevFD6818.c`), same base, same state
/// table. `REG_VOLUME` (0x48) is switched alongside it.
...
/// Extra REG 0x47 bit `set_af_out()` ORs in for RX audio while demodulating
/// AM, based on this board's own disassembly of the stock firmware's
/// `Rfic_SetAfout()`, and matching K6's working `fd6818.rs` (BK4819) driver.
...
/// Extra REG 0x47 bit `set_af_out()` ORs in for RX audio while demodulating
/// `Modulation::Usb`/`Cw`. See `AM_AF_OUT_BIT`'s doc comment.
...
/// `DEVIATION_VALUE` normally). Not sourced from a confirmed BK4829
/// reference (the driver this was cross-checked against has no scramble
/// feature) -- carried over from BK4819 unverified.
...
/// BK4829's init-time REG_37 words. Both bit15+bit4 set relative to
/// BK4819's `0x1D0F`/`0x1F0F`.
const POWER_UP_VALUE: u16 = 0x9D1F;
const POWER_RX_VALUE: u16 = 0x9F1F;
...
const DEVIATION_VALUE: u16 = 0x3516;
const MIC_SENS_VALUE: u16 = 0xE920;
...
impl<'a> Fd6818<'a> {
    ...
    fn select_band(&mut self, syst: &mut SYST) {
        let uhf = self.band_uhf;
        self.set_gpio_bit(syst, GPIO5, !uhf);
        self.set_gpio_bit(syst, GPIO6, uhf);
    }

    pub fn set_tx_band(&mut self, syst: &mut SYST) {
        let uhf = self.band_uhf;
        self.set_gpio_bit(syst, GPIO3, !uhf);
        self.set_gpio_bit(syst, GPIO4, uhf);
    }

    pub fn set_band_uhf(&mut self, syst: &mut SYST) {
        self.band_uhf = true;
        self.select_band(syst);
    }

    pub fn set_band_vhf(&mut self, syst: &mut SYST) {
        self.band_uhf = false;
        self.select_band(syst);
    }

    pub fn set_band_quiet(&mut self, uhf: bool) {
        self.band_uhf = uhf;
    }

    pub fn set_tx_band_off(&mut self, syst: &mut SYST) {
        self.set_gpio_bit(syst, GPIO3, false);
        self.set_gpio_bit(syst, GPIO4, false);
        self.set_gpio_bit(syst, GPIO5, false);
        self.set_gpio_bit(syst, GPIO6, false);
    }

    /// Enables the PA stage at the calibrated APC target, paired with
    /// BK4829's fixed band-dependent driver gain.
    pub fn pa_enable(&mut self, syst: &mut SYST, _power: Power) {
        let gain = if self.band_uhf {
            PA_GAIN_UHF
        } else {
            PA_GAIN_VHF
        };
    ...
    /// called on every PTT release / RF shutdown.
    ...
    pub fn set_scramble(&mut self, syst: &mut SYST, group: u8) {
        ...
        if group == 0 {
            ...
            self.write_reg(syst, REG_DEVIATION, DEVIATION_VALUE);
        } else {
    ...
    /// config after turning this back off.
    ...
    /// repeating bitstream). Only checks normal polarity -- never reports
    /// an inverted-polarity match. Returns the plain DCS code.
    ...
    /// Sets REG_POWER (0x37) to the RX-specific value.
    ...
    fn write_bits(&mut self, syst: &mut SYST, byte: u8) {
        ...
        while mask != 0 {
            board::set_fd6818_sck(self.gpioa, false);
            board::set_shared_sda(self.gpioa, byte & mask != 0);
            delay::us(syst, BIT_DELAY_US);
            board::set_fd6818_sck(self.gpioa, true);
            delay::us(syst, BIT_DELAY_US);
            mask >>= 1;
        }
    }
    ...
    pub fn chip_id(&mut self, syst: &mut SYST) -> u16 {
        self.read_reg(syst, REG_SOFT_RESET)
    }
    ...
    pub fn init(&mut self, syst: &mut SYST) {
        ...
        // RX AGC
        self.write_reg(syst, REG_RXAGC_1, 0x03DF);
        self.write_reg(syst, REG_RXAGC_2, 0x03DB);
        self.write_reg(syst, REG_RXAGC_3, 0x033A);
        ...
        self.write_reg(syst, REG_2A, 0x6600);
        ...
        self.write_reg(syst, REG_VOLUME, 0x33A8);
        ...
        self.write_reg(syst, 0x1C, 0x07C0);
        self.write_reg(syst, 0x1D, 0xE555);
        self.write_reg(syst, 0x1E, 0x4C58);
        self.write_reg(syst, 0x28, 0x0B40);
        self.write_reg(syst, 0x29, 0xAA00);
        self.write_reg(syst, 0x2C, 0x1822);
        self.write_reg(syst, 0x2F, 0x9890);
        self.write_reg(syst, 0x53, 0x2028);
        self.write_reg(syst, 0x7E, 0x303E);
        self.write_reg(syst, 0x46, 0x600A);
        self.write_reg(syst, 0x4A, 0x5430);

        self.write_reg(syst, REG_SUBAUDIO_FREQ, 0x61CE);

        // GPIO initial state
        self.write_reg(syst, REG_GPIO, 0x9000);
        self.write_reg(syst, REG_WORK_MODE, 0x0000);
    }
}
