...
use crate::board;
use crate::drivers::fd6818b::{AfOutState, Fd6818};
pub use crate::drivers::fd6818b::{Modulation, Power, RawTone, SubAudio};
...
/// ADC channel for VOX mic level (PA0 = ADC_IN0).
pub const VOX_ADC_CHANNEL: u8 = 0;
/// ADC channel for battery voltage (PA1 = ADC_IN1).
...
impl FreqRanges {
    ...
    /// The FD6818B's own synthesizer capability, from datasheet -- same
    /// underlying BK48xx PLL as K6's FD6818, chip-generic.
...
impl AniConfig {
    pub fn from_raw(machine_id: [u8; 3], separator_idx: u8) -> Self {
...
impl<'a> Radio<'a> {
    ...
    pub fn park_for_fm(&mut self, syst: &mut SYST) {
        ...
        board::set_tx_led(self.gpioa, false);
    ...
    /// pushing it into the FD6818B driver. Returns `true` if a calibration
    /// address was found.
    pub fn apply_pa_calibration(
        &mut self,
        storage: &mut crate::device::storage::Storage<'_>,
        freq_hz: u32,
        power: Power,
    ) -> bool {
    ...
    const MIC_MUTE_SETTLE_US: u32 = 500;

    pub fn read_mic_level(&mut self, syst: &mut SYST) -> u8 {
        let was_on = self.speaker_on;
        if was_on {
            self.set_speaker(false);
            delay::us(syst, Self::MIC_MUTE_SETTLE_US);
        }
        let level = (self.adc.read_channel(VOX_ADC_CHANNEL) >> 4) as u8;
        if was_on {
    ...
    fn assert_band(&mut self, syst: &mut SYST, band: Band) {
        match band {
            Band::Uhf => self.fd6818.set_band_uhf(syst),
            Band::Vhf => self.fd6818.set_band_vhf(syst),
        }
    }
    ...
    /// and so does any `tone_index` outside `0..=45`. Sidetone only (never
    /// keys the transmitter) -- called once at boot, before the radio has
    /// entered RX. See `bfk6-fw/src/device/radio.rs`'s copy of this function
    /// for the full rationale (unchanged here).
    pub fn play_boot_tune(
        &mut self,
        syst: &mut SYST,
        tune: &[(u8, u8); 48],
        mut should_abort: impl FnMut(&mut SYST) -> bool,
    ) -> bool {
        ...
        for &(tone_index, duration) in tune {
            ...
            let mut remaining_ms = duration as u32 * BOOT_TUNE_PERIOD_MS;
            while remaining_ms > 0 {
                let slice = remaining_ms.min(BOOT_TUNE_PERIOD_MS);
                delay::ms(syst, slice);
                remaining_ms -= slice;
                if should_abort(syst) {
                    self.set_speaker(false);
    ...
    pub fn poll_squelch(&mut self, syst: &mut SYST, debounce_ticks: u8) -> bool {
        ...
        if rssi_open != self.rssi_open {
            ...
            if self.rssi_debounce >= debounce_ticks {
                ...
                board::set_rx_led(self.gpioa, rssi_open);
            }
        } else {
            self.rssi_debounce = 0;
        }
    ...
    pub fn enter_rx(&mut self, syst: &mut SYST) {
        ...
        self.assert_band(syst, self.band());
    ...
    /// Lightweight frequency retune while staying in RX mode. Does not reset
    /// squelch, subaudio, or audio path, only programs the PLL, reasserts
    /// the front-end band, and re-enters RX.
    pub fn retune_rx(&mut self, syst: &mut SYST, freq_hz: u32, wide: bool) {
        self.cfg.freq_hz = freq_hz;
        self.cfg.wide_band = wide;
        self.assert_band(syst, Self::band_of(freq_hz));
    ...
    pub fn enter_tx(&mut self, syst: &mut SYST) -> bool {
        ...
        self.set_speaker(false);
        board::set_rx_led(self.gpioa, false);
        board::set_tx_led(self.gpioa, true);
        self.fd6818.rf_off(syst);
        self.fd6818.idle(syst);
        ...
        self.fd6818
            .set_band_quiet(Self::band_of(self.cfg.tx_freq_hz) == Band::Uhf);
        self.fd6818.set_tx_band(syst);
        self.tx_state = true;
        true
    }
    ...
    pub fn tune_search_candidate(&mut self, syst: &mut SYST, freq_hz: u32, uhf_path: bool) {
        ...
        self.assert_band(syst, if uhf_path { Band::Uhf } else { Band::Vhf });
        self.fd6818.rx_on(syst);
    }
